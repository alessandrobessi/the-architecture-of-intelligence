...
to mean. It's tempting, seeing a number like 92%, to treat a high
benchmark score as meaning a model is definitely better at real-world use
of that skill — but contamination and Goodhart's-law-style overfitting to
a benchmark's specific format can both inflate a score without any
corresponding improvement in the broader, real-world capability the
benchmark was meant to approximate. A benchmark score is a proxy for a
capability, not the capability itself — worth checking for contamination
and format-specific overfitting before treating a comparison as settled,
the same way the hiring manager's quiz score stopped meaning "good
programmer" the moment candidates started optimizing for the quiz
specifically, even though the number on the screen looked exactly the
same as before.
...
contamination: the exact test questions leaking into a model's training
data (Chapter 9), inflating its score without reflecting genuine
...
But it's tempting to treat this as just as trustworthy as having a human
expert review something directly — it isn't, quite. A judge model
inherits its own blind spots: it can hallucinate a justification for a
wrong verdict the same way Chapter 15 described for any other generated
claim, and outputs can be specifically styled to appeal to that
particular judge's known preferences rather than genuinely being better.
LLM-as-judge is a useful, cheap proxy that often correlates with human
judgment at scale — valuable in combination with human evaluation and
diverse benchmarks, not a guaranteed, interchangeable substitute for
either, the same way a trusted colleague's quick read of a report is
genuinely useful and often right, but isn't the same guarantee as a full
independent audit, and treating it as one invites exactly the errors an
audit exists to catch.

This is why AI providers publish tables of many separate named benchmarks
rather than one single score — no single number is trustworthy enough to
stand alone, for exactly the reasons covered above. It's tempting to
assume that if a new model version scores higher on the overall
benchmark, it's strictly better at everything the old model was good at —
but an aggregate score averages across many different sub-skills, and a
genuine regression on one specific capability can be entirely masked by
improvement on others, even while the single headline number goes up,
the same way a student's overall exam average can rise even while their
specific grasp of one topic quietly gets worse, if they improved enough
elsewhere to cover for it. One aggregate score can hide capability-
specific regressions — meaningful evaluation tracks performance across
many distinct skills, not just a single combined figure.

It's also why practitioners are consistently urged to build evaluation
sets specific to their own actual use case rather than relying solely on
general public benchmarks, which may not reflect their domain and may
already be contaminated or specifically optimized against by model
developers. And it's the direct reason "LLM-as-judge" pipelines have
become a common piece of real product evaluation workflows: not because
they're perfectly reliable, but because they make frequent, cheap
evaluation possible at a scale exhaustive human review never could
reach. A serious evaluation plan for a real product tests more than the
model's raw answers: whether retrieval (Chapters 17–18) actually
surfaces the right passage, whether a tool-using agent (Chapters 21–22)
picks the right tool and recovers sensibly when a step fails, and how
the complete pipeline performs end to end — not just a benchmark score
for the model sitting at its core, which can look excellent while the
surrounding system still lets users down.
...
- Search for "benchmark contamination" or "data contamination LLM evaluation" for more on the failure mode described above.
- Search for "LLM-as-judge" for concrete, current examples of the evaluation pattern described above.
