...
starting points as they read (a caveat covered later in this chapter, and
the subject of Chapter 11 in full). The map metaphor is the same; the
resident has just moved from "words" to "tokens," and turns out not to
stay in one place.
...
together. "Cat" and "stock market" end up far apart. It's tempting to
picture something like a stored dictionary definition sitting behind each
of these points — but nowhere in an embedding is there a stored sentence
explaining what a word means. There is only a location in space, learned
from patterns of use, the same way a city's position on a map is just a
location, not a paragraph about the city. Usually this tracks similar
meaning too — but "used similarly" and "means the same thing" aren't quite
the same test, and the embedding only ever sees the first one, as the next
paragraph shows.
...
placed near each other. It's tempting to assume that "similar" here just
means "synonym" — but words end up close together whenever they're used in
similar contexts, which captures far more than synonymy. Antonyms like
"hot" and "cold," for instance, often end up relatively close too, since
they're used in nearly identical grammatical contexts ("the water is
___"). Try it yourself, before reading on: would you expect "hot" to sit
closer to "cold," or closer to "sandwich," on this map? Most people's
first instinct is "sandwich" — hot and cold feel like opposites, so surely
they shouldn't be neighbors. But "hot" is actually closer to "cold": both
fill the same slot in the same kinds of sentences ("the water is ___,"
"turn the ___ tap"), while "sandwich" almost never does. If your first
guess was "sandwich," you were reasoning about *meaning-as-agreement*
rather than *meaning-as-use* — exactly the gap this section is pointing
at, caught in the act. On the culture-and-economy city map from the
opening, two rival neighboring capitals might sit close together despite
being political opposites — closeness there tracked "type of place," not
"gets along with." Closeness in an embedding space reflects similarity of
*use and context*, which is a broader and sometimes subtler relationship
than "means the same thing."
...
subject of Chapter 11. It's tempting, then, to think of an embedding as a
word's one true, permanent representation of meaning, the same wherever it
appears — but what this chapter describes is only a token's starting
location, learned from its typical usage across all contexts. This
chapter's embedding is a token's general-purpose starting point, not its
final, in-context meaning — the final meaning is computed fresh for every
sentence, the same way a person's home address is a fixed, general-purpose
location, but where they actually are right now, in context, changes
throughout the day. This chapter describes the home address; Chapter 11
describes where the token actually is right now. Everything in this
chapter describes a token's initial position, not its final, in-context
one.
...
- The model does not store definitions — it stores locations, learned from context of use.
