...
Four ideas, in the order you need them, build toward that relationship.
...
It's tempting to assume that a computer processing information this way
must, on some level, understand what that information means — but
computation, by its formal definition, requires only mechanical
rule-following, not comprehension; a telegraph relay processes Morse code
perfectly without understanding a word of English, the same way a vending
machine correctly processes coin-shaped-and-weighted metal without knowing
what money is. Whether or not any "understanding" is happening in a
computer is a separate, much harder question this book will return to.
...
all carries a lot of information. It's tempting to assume that more
information just means more data — that a longer message always contains
more information than a shorter one — but that isn't how the word is being
used here: a long, entirely predictable message ("it will be exactly like
every other day this month," stretched out to a hundred pages) can carry
almost no information, because it tells you almost nothing you didn't
already expect. Information content depends on how surprising the message
is, not on its length.
